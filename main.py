...
class Bot:
    ...
    def get_user_info(self, user_id):
        ...
        try:
            ...
            return False                            # Исключения должны влиять на логику исполнения программы (замечание проверяющего №1)
        else:
            for i in list_1:
                ...
                try:
                    ...
                    self.write_message(user_id, 'Ошибка получения города. Введите в свой профиль город и попробуйте заново')
                    return False
    ...
    def find_insert_users(self, user_id, offset_1):
        ...
        try:
            ...
            return 'Error'                              # Исключения должны влиять на логику исполнения программы (замечание проверяющего №1)
        list_1 = dict_1['items']
        for person_dict in list_1:
            if person_dict.get('is_closed') == False:
                person = (person_dict.get('first_name'), person_dict.get('last_name'), str(person_dict.get('id')),
                          'vk.com/id' + str(person_dict.get('id')))
    ...
    def find_persons(self, user_id, person):
        ...
        insert_data_orm(person[2])                   # А в бд отправляйте только ид просмотренных анкет (замечание проверяющего №2)


    def number_of_persons(self, user_id, persons, offset):   # механика на случай, если найденные анкеты закончились (замечание проверяющего №5)
        count = 0
        if persons == []:
            return True
        else:
            for i in persons:
                count += 1
            if offset >= count:
                self.write_message(user_id, 'Анкеты закончились')
                return False
            else:
                return True
